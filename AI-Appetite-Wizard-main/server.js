...
app.get("/ingredients", (req, res) => {
  const ingredients = ['Eggs', 'Cheese, shredded', 'Herbs chives, parsley', 'Salt', 'Pepper', 'Salmon', 
...
app.get("/users/favorites/:username", (req, res) => {
  ...
  p.then((user) => {
    ...
    console.log(err);
    res.status(500).send("There was an issue getting the user's favorites "+ err);
  });
});
//for returning the full data favorites not just id #s
app.get("/users/favoritesData/:username", (req, res) => {
  const username = req.params.username;
  console.log(req.url + " " + username);
  // Find the user in the database
  let p = Users.findOne({
    username: { $regex: new RegExp("^" + username, "i") }, }).exec();
  // Send the user's favorites
  p.then((user) => {
      if (!user) {
        return res.status(404).end( "User not found" );
		  ...
		  const favoriteRecipes = Recipes.find({ _id: { $in: user.favorites },
		  }).exec();
		  favoriteRecipes.then((data) => {
...
app.post("/users/add/favorite", (req, res) => {
  ...
  p.then((user) => {
    ...
    console.log(err);
    res.status(500).send("Error adding recipe to favorites."+ err); // 500 is internal server error
  });
});
